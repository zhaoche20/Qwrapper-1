 ...
 */
public class Wrapper_gjsairlx002 implements QunarCrawler {
//    private static final Logger logger = LoggerFactory.getLogger("CrawlerLog");
    private static final String CODEBASE = "gjsairlx002";
    ...
    public String getHtml(FlightSearchParam param) {
        ...
        try {
            ...
            temp=param.getRetDate().split("-");
            String inbound=temp[2]+"."+temp[1]+"."+temp[0];
            get=new QFGetMethod("http://tracking.mlsat03.de/swiss/api/index_phg.php?kid=995&dlid=1274&conid=8626&fwid=182053&pid=10lGaM&pwd=yQkPQT5K&origin="+param.getDep()+"&destination="+param.getArr()+"&outbound="+outbound+"&inbound="+inbound+"&adults=1&children=0&infants=0&language=zh&country=cn");
...
//                logger.warn("{},{}", new Object[] { CODEBASE, "NO_RESULT"});
            ...
            try {
                ...
                for(int i=0;i<airlines.length;i++)
                {
                    ...
                    String depInfoRaw= StringUtils.substringBetween(airlines[i], "<Outbound>", "</Outbound>");//去程部分
                    String retInfoRaw= StringUtils.substringBetween(airlines[i], "<Inbound>", "</Inbound>");//回程部分
                    ...
                    repeatcode=flight;
                    List<FlightSegement> info=getFlightSegement(depInfoRaw);
                    List<FlightSegement> retInfo=getFlightSegement(retInfoRaw);
                    ...
                    Date retDate=sdf.parse(param.getRetDate());
...
//                        "PARSING_FAIL"});
    ...
    private List<FlightSegement> getFlightSegement(String info)
    {
        List<FlightSegement> infoFlightSegement= Lists.newArrayList();
        String[] flightSegement= StringUtils.substringsBetween(info, "<FlightSegment>", "</FlightSegment>");
        for(String flightSegementPart:flightSegement)
        {
            ...
            fs.setCompany(flightNoSegement.substring(0,2));
            infoFlightSegement.add(fs);

        }
        return infoFlightSegement;
    }
    ...
    public static void main(String[] args)
    {
        ...
        f.setRetDate("2014-07-30");
        Wrapper_gjsairlx002 wr = new Wrapper_gjsairlx002();
